...
- transport_order: 交通出行订单截图（滴滴/高铁/机票/公交/地铁等）
...
## 返回格式（严格JSON，不要包含任何其他文本）
{
  ...
  "expense": {
    "amount": 328.00,
    "merchant": "海底捞",
    "occurredAt": "2026-08-17T19:30:00",
    "categoryKey": "food"
  },
  "orderItems": [],
  "confidence": {
...
## 重要规则
- 识别不到的字段返回null
- food_photo类型：expense返回null，orderItems返回空数组
- 禁止编造任何数据
- amount使用数字（元），不要包含货币符号
- confidence范围0-1，低于0.5表示不太确定
- 如果图片无法识别（如风景照、自拍等），返回 {"ok":false,"imageType":"unknown","expense":null,"orderItems":[],"confidence":{},"warnings":["无法识别账单信息"]}
